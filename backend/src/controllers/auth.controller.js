...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        if (existUser && existEmail) { return res.status(404).send({ message: "Đã tồn tại người dùng" }) }
        else {
        ...
        res.status(500).send({ message: "Thất bại khi tạo người dùng" })
    }
}
...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        if (!user) { return res.status(401).send({ message: "Người dùng không tồn tại!" }) }

        const validPassword = await bcrypt.compare(password, user.password)
        if (!validPassword) { return res.status(404).send({ message: "Sai tài khoản hoặc mật khẩu!" }) }
        const token = jwt.sign(
